...
function GamePageTab({ org, onSlugUpdate }) {
  ...
  // Sync if org slug changes from parent
  useEffect(() => {
    setSlug(org?.slug || '');
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          @media print {
            body { padding: 0; }
            .flyer { box-shadow: none; border-radius: 0; border: none; }
          }
        </style>
      </head>
      <body>
        <div class="flyer">
          ...
          <div class="promo">
            <p>${promoText.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
          </div>
        ...
        <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-3">
          ...
          {portalUrl ? (
            <a href={portalUrl} target="_blank" rel="noreferrer"
              className="flex-1 text-sm font-mono text-purple-600 hover:underline truncate">
              {portalUrl}
            </a>
          ) : (
        ...
        <div className="border border-dashed border-purple-200 rounded-xl p-4 bg-purple-50/40">
          ...
          {editing ? (
            <div className="flex items-center gap-2">
              <div className="flex items-center flex-1 bg-white border border-purple-300 rounded-xl overflow-hidden">
                <span className="px-3 text-xs text-gray-400 whitespace-nowrap border-r border-gray-200 py-2.5">
              ...
              <button onClick={handleSaveSlug} disabled={saving}
                className="text-xs px-3 py-2 rounded-xl bg-green-600 text-white hover:bg-green-700 font-semibold disabled:opacity-50">
                {saving ? 'Saving...' : 'Save'}
              </button>
              <button onClick={() => { setEditing(false); setEditSlug(slug); }}
                className="text-xs px-3 py-2 rounded-xl bg-gray-200 text-gray-600 hover:bg-gray-300">
                Cancel
              </button>
            </div>
          ) : (
            <p className="text-xs text-gray-500">
              Current slug: <span className="font-mono font-semibold text-purple-700">{slug || 'not set'}</span>
              <span className="ml-2 text-gray-400">— only lowercase letters, numbers, hyphens allowed</span>
            </p>
          )}
        </div>
      </div>
      ...
      {portalUrl && (
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="card p-5">
            ...
            />
            <p className="text-xs text-gray-400 mt-1">{promoText.length} chars &bull; Tip: use emojis to make it eye-catching 🚀</p>
            <button onClick={handlePrint} className="btn-primary w-full mt-3 text-sm">
              🖨️ Preview & Print Flyer
            </button>
          </div>
        </div>
      )}
...
export default function CustomersPage() {
  ...
  const [tab, setTab]   = useState('customers'); // 'customers' | 'gamepage'
  ...
  // When slug is updated, sync back into auth store so sidebar reflects new URL
