    ...
    <div className="flex gap-0.5">
      {Array.from({ length: max }, (_, i) => (
        <span key={i} className="text-base" style={{
          filter: i < value ? 'drop-shadow(0 0 2px rgba(250,204,21,0.6))' : 'grayscale(1) opacity(0.3)',
        }}>⭐</span>
      ))}
    </div>
  );
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="card p-3 md:p-4 flex items-center gap-2 md:gap-3">
          ...
          <div><p className="text-xl md:text-2xl font-black text-gray-900">{overallAvg ? overallAvg.toFixed(1) : '—'}</p><p className="text-[10px] md:text-xs text-gray-400">Avg Rating</p></div>
        </div>
        <div className="card p-3 md:p-4 flex items-center gap-2 md:gap-3">
          <div className="w-9 h-9 rounded-xl bg-green-50 flex items-center justify-center text-lg flex-shrink-0">💬</div>
          <div><p className="text-xl md:text-2xl font-black text-gray-900">{data.total}</p><p className="text-[10px] md:text-xs text-gray-400">Total Reviews</p></div>
        </div>
        <div className="card p-3 md:p-4 flex items-center gap-2 md:gap-3">
          <div className="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center text-lg flex-shrink-0">🏆</div>
          <div><p className="text-xl md:text-2xl font-black text-gray-900">{totalWith5}</p><p className="text-[10px] md:text-xs text-gray-400">Perfect 5★</p></div>
        </div>
      </div>
      ...
      <div className="card p-3 mb-4">
        <div className="flex flex-col sm:flex-row gap-2 flex-wrap">
          <input className="input flex-1 min-w-0" placeholder="Search by name or phone"
            value={search} onChange={e => setSearch(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1">
              <span className="text-xs text-gray-400 whitespace-nowrap">From</span>
              <input className="input w-32" type="date" value={from} onChange={e => setFrom(e.target.value)} />
            </div>
            <div className="flex items-center gap-1">
              <span className="text-xs text-gray-400 whitespace-nowrap">To</span>
              <input className="input w-32" type="date" value={to} onChange={e => setTo(e.target.value)} />
            </div>
      ...
      {/* Table — overflow-x-auto fixes mobile hidden content */}
                ...
                return (
                  <tr key={row._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-semibold text-gray-800 text-sm whitespace-nowrap">{row.customerId?.name || '—'}</p>
                      <p className="text-xs text-gray-400">{row.customerId?.phone || ''}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className="text-base">{GAME_ICONS[row.gameId?.key] || '🎮'}</span>
                        <span className="text-xs font-medium text-gray-600 whitespace-nowrap">{row.gameId?.name || '—'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3"><RatingBadge value={row.feedback?.gameRating} /></td>
                    <td className="px-4 py-3"><RatingBadge value={row.feedback?.offerRating} /></td>
                    <td className="px-4 py-3"><RatingBadge value={row.feedback?.enjoymentRating} /></td>
                    <td className="px-4 py-3"><AvgCircle value={a} /></td>
                    ...
                    <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">
                      {row.feedback?.submittedAt ? format(new Date(row.feedback.submittedAt), 'dd MMM yy, HH:mm') : '—'}
                    </td>
                    <td className="px-4 py-3">{a && <StarDisplay value={Math.round(a)} />}</td>
                  </tr>
                );
              })}
