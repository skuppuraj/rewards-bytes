    ...
    <div className="max-w-md mx-auto px-4 py-6">
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-6 text-center" style={{background: 'linear-gradient(135deg, var(--brand-btn, #6366f1), var(--brand-btn2, #8b5cf6))'}}>
          <div className="text-5xl mb-3">{game.key === 'spin_wheel' ? '🎡' : '🃏'}</div>
        ...
        <div className="p-5">
                ...
                {orgGame.assignedOffers.map(o => (
                  <div key={o._id} className="flex items-center gap-2 p-2 bg-purple-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-purple-600">{o.discountType === 'percentage' ? `${o.discountValue}% off` : `₹${o.discountValue} off`}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleStart} disabled={starting} className="flex-2 flex-1 py-3 rounded-xl text-white font-bold text-sm" style={{background: 'var(--brand-btn, #6366f1)'}}>
